...
score.dt <- score.dt[algorithm == "tuned" & train.subsets == "all"]
...
for (taxa in unique(score.dt$task_id)) {
  ...
  if (nrow(fuser_results) > 0) {
    ...
    typeof(fuser_model$learner$model$beta) 
    #fuser_model$learner$model$beta
    fuser_matrix <- as.matrix(fuser_model$learner$model$beta)
